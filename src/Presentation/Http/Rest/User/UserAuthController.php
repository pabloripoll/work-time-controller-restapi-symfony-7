...
class UserAuthController extends AbstractController
{
    ...
    public function __construct(
        ...
        private EmployeeAccessLogRepositoryInterface $EmployeeAccessLogRepo
    ) {}
    ...
    public function refresh(Request $request, #[CurrentUser] User $user): JsonResponse
    {
        try {
            ...
            $accessLog = $user->isAdmin()
                ...
                : $this->EmployeeAccessLogRepo->findByToken($token);
    ...
    #[IsGranted('IS_AUTHENTICATED_FULLY')]
    public function whoami(#[CurrentUser] User $user, GetUserByIdHandler $handler): JsonResponse
    {
        try {
            $query = new GetUserByIdQuery($user->getId());
